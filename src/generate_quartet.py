...
from quartet import DEFAULT_SETTINGS, QuartetImage
...
QuartetImage(**{key: val for key, val in data.items() if val is not None}).convert()
# qi = QuartetImage(**{key: val for key, val in data.items() if val is not None})
# print(qi.__dict__)